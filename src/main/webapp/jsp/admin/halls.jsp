        ...
        <c:choose>
            <c:when test="${showHalls}">
                        ...
                        <tr>
                            ...
                            <td class="button-td" style="width: 9rem">
                                <form action="controller" method="post">
                                    <fmt:message bundle="${local}" var="delete_hall_label" key="admin.button_labels.delete_hall" />
                                    <input type="hidden" name="hall_id" value="${hall.id}">
                                    <input type="hidden" name="command" value="deleteHall">
                                    <input type="hidden" name="sendRedirect" value="true">
                                    <button class="btn btn-primary btn-block" type="submit"><c:out value="${delete_hall_label}"></c:out></button>
                                </form>
                            </td>
                        </tr>
                    </c:forEach>
                ...
                <div class="paging text-center">
                    ...
                    <ul class="pagination">
                        <li class="page-item <c:if test="${currentPage == 1}">disabled</c:if>"><a class="page-link" href="controller?command=listHalls&page=${currentPage - 1}"><c:out value="${pagination_previous}"></c:out></a></li>
                        <c:forEach begin="1" end="${noOfPages}" var="i">
                            <c:choose>
                                ...
                                <c:otherwise>
                                    <li class="page-item"><a  class="page-link" href="controller?command=listHalls&page=${i}">${i}</a></li>
                                </c:otherwise>
                            </c:choose>
                        </c:forEach>
                        ...
                        <li class="page-item <c:if test="${currentPage == noOfPages}">disabled</c:if>" ><a class="page-link" href="controller?command=listHalls&page=${currentPage + 1}" aria-disabled="<c:out value="${currentPage == noOfPages}"></c:out>"><c:out value="${pagination_next}"></c:out></a></li>
                            <%--</c:if>--%>
                    </ul>
                </div>
                <%--Pagination--%>
                <form action="controller" method="get">
                    ...
                    <button class="btn btn-primary btn-block" type="submit"><c:out value="${create_hall_label}"></c:out></button>
                </form>
            </c:when>
            <c:when test="${ sendRedirectHalls}">
                <fmt:message bundle="${local}" key="admin.button_labels.back_to_halls_list" var="back_to_halls_label"/>
                <a href="controller?command=listHalls" class="btn btn-primary btn-block"><c:out value="${back_to_halls_label}"></c:out></a>
            </c:when>
        </c:choose>
    </div>
</main>
