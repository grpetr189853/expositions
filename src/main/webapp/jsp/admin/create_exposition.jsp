...
<html>
<head>
    <title>Admin Create Exposition</title>
        ...
        <c:choose>
            <c:when test="${showCreateExposition}">
                <form action="controller" method="post" id="expositionForm" enctype="multipart/form-data">
                    ...
                    <div class="mb-3 row">
                        <fmt:message bundle="${local}" var="ticket_price_label" key="create_exposition.label.ticket_price" />
                        <label for="ticket_price">${ticket_price_label}</label>
                        <input type="text" name="ticket_price" class="form-control" id="ticket_price">
                    </div>
                    <div class="mb-3 row">
                        <fmt:message bundle="${local}" var="tickets_count_label" key="create_exposition.label.tickets_count" />
                    ...
                    <div class="mb-3 row">
                        ...
                        <select class="form-select" multiple aria-label="multiple select example" name="halls" id="halls">
                            <c:forEach items="${ requestScope.halls}" var="hall">
                                <option value="<c:out value="${hall.id}"></c:out>"><c:out value="${hall.hallName}"></c:out></option>
                            </c:forEach>
                        </select>
                    </div>
                    <div class="mb-3 row">
                    ...
                    <fmt:message bundle="${local}" var="create_button_label" key="create_exposition.button_label.create" />
