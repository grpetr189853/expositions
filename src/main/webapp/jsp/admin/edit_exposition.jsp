        ...
        <c:choose>
            <c:when test="${showEditExposition}">
                <form action="controller"  method="post" id="expositionForm" enctype="multipart/form-data">
                    ...
                    <div class="mb-3 row">
                        <fmt:message bundle="${local}" var="ticket_price_label" key="edit_exposition.label.ticket_price" />
                        <label for="ticket_price">${ticket_price_label}</label>
                        <input type="text" name="ticket_price"  class="form-control" value="${exposition.ticketPrice}" id="ticket_price">
                    </div>
                    <div class="mb-3 row">
                        <fmt:message bundle="${local}" var="tickets_count_label" key="edit_exposition.label.tickets_count" />
                        <label for="tickets_count">${tickets_count_label}</label>
                        <input type="text" name="tickets_count"  class="form-control" value="${exposition.ticketsCount}" id="tickets_count">
                    </div>
                    <div class="mb-3 row">
                        ...
                        <input type="date" name="date_in"  class="form-control" value="${exposition.dateIn}" id="date_in">
                    </div>
                    <div class="mb-3 row">
                        <fmt:message bundle="${local}" var="end_date_label" key="edit_exposition.label.end_date" />
                        <label for="date_out">${end_date_label}</label>
                        <input type="date" name="date_out"  class="form-control" value="${exposition.dateOut}" id="date_out">
                    </div>
                    <div class="mb-3 row">
                        <fmt:message bundle="${local}" var="halls_label" key="edit_exposition.label.halls" />
                        <label for="halls">${halls_label}</label>
                        <select class="form-select" multiple aria-label="multiple select example" name="halls" id="halls">
                            <c:forEach  begin="0" end="${fn:length(halls) - 1}" var="index">
                                <option <c:choose><c:when test="${selectedHallsChecked.get(index) == 1}"><c:out value="selected"/></c:when><c:otherwise></c:otherwise></c:choose> value="<c:out value="${halls.get(index).id}"></c:out>"><c:out value="${halls.get(index).hallName}"></c:out></option>
                            </c:forEach>
                        </select>
                    </div>
                    <div class="mb-3 row">
